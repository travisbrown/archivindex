...
# Requires the Python `surt` package (`pip install surt`) and warcio.js (`npm install warcio`); rows
# omitted from each fixture are described in its header and must be filtered by hand.
set -e
cd "$(dirname "$0")"
python3 -c '
import sys, surt
