...
/// path becomes `/`. When `collapse_slashes` is set, empty segments other than the one that forms a
/// trailing slash are dropped, so `/a//b/` becomes `/a/b/`.
///
/// [surt]: https://github.com/internetarchive/surt
...
/// four decimal or octal parts follow `inet_aton`, so `10.0.258` is `10.0.1.2` and `017.0.0.1` is
/// `15.0.0.1`. Ordinary dotted-quad addresses are returned unchanged.
...
/// Encode a non-ASCII host label as `xn--` plus RFC 3492 Punycode.
///
/// Returns `None` for ASCII bytes that require escaping or for arithmetic overflow. This does not
/// perform full IDNA validation.
#[expect(
