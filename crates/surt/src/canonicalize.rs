...
//! `http://https://...`), lowercases the scheme, host, path, and query, drops the user information
//! and fragment, drops a default port and an empty query, resolves path dot segments, strips
//! trailing dots from the host, rewrites numeric IPv4 hosts as dotted quads, and encodes non-ASCII
//! host labels with Punycode. The flags on [`Canonicalizer`] select the remaining rules.
//!
...
impl Canonicalizer {
    ...
    ///
    /// These keep a trailing slash and existing escapes, which are lowercased with the rest of the
    /// URL. Characters that a WHATWG URL parser would escape are left as they are.
    ///
    ...
    /// A preset with all optional normalization rules disabled.
    ///
    /// The common repairs and normalization still apply; see [`Canonicalizer`]. Use
    /// [`Url::heritrix`] to render the result with [Heritrix]'s scheme and parentheses.
    ///
    ...
    ///
    /// This is exactly [`Canonicalizer::canonicalize`] followed by [`Url::surt`]. IPv6 hosts are
    /// written without brackets, as the Wayback Machine does. Use
    /// [`Canonicalizer::surt_ipv6_bracketed`] to keep the brackets.
    ///
    /// # Errors
    ///
    ...
    /// Canonicalize a URL and transform it into a SURT key, keeping an IPv6 host's brackets.
    ///
    /// This differs from [`Canonicalizer::surt`] only for IPv6 hosts; see [`Url::surt_ipv6_bracketed`].
    ///
    /// # Errors
    ///
    /// Fails when the URL cannot be split into components; see [`url::Error`].
    pub fn surt_ipv6_bracketed(&self, url: &str) -> Result<Surt<'static>, url::Error> {
        Ok(self.canonicalize(url)?.surt_ipv6_bracketed())
    }
    ...
    fn push_host(self, output: &mut String, host: &str) {
        ...
        // Unlike the Python library, `www` is stripped before the numeric check, so that `www.0`
        // and `0.0.0.0` reach the same key on every pass.
...
mod tests {
    ...
    fn wayback_keys_are_canonical(#[strategy = prop::url()] url: String) {
    ...
    #[proptest::property_test]
    fn wayback_is_idempotent(#[strategy = prop::url()] url: String) {
        ...
        let key = first.surt_ipv6_bracketed();
        let via_key = Canonicalizer::WAYBACK
            .surt_ipv6_bracketed(&key.url_with_scheme(first.scheme()).to_string())
            .unwrap();
    ...
    #[proptest::property_test]
    fn warcio_and_heritrix_are_idempotent(#[strategy = prop::url()] url: String) {
    ...
    /// Bracket stripping is a rendering choice, so the same canonicalizer produces both forms.
    #[test]
    fn renders_ipv6_hosts_bare_or_bracketed() {
        let url = "http://[2001:DB8::1]:8080/p";
        let rules = Canonicalizer::WAYBACK;

        assert_eq!(rules.surt(url).unwrap().as_str(), "2001:db8::1:8080)/p");
        assert_eq!(
            rules.surt_ipv6_bracketed(url).unwrap().as_str(),
            "[2001:db8::1]:8080)/p"
        );
    }
}
