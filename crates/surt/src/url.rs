...
//! host delimiters, and port. It rejects ASCII whitespace and control characters but does not
//! validate domain labels or the contents of bracketed hosts against a URL standard. It can render
//! the host-reordering transforms [`Url::surt`], [`Url::heritrix`], and [`Url::ssurt`]. A
//! [`Canonicalizer`] first applies a convention's normalization rules so that equivalent URLs
//! render the same.
...
pub enum Error {
    ...
    /// The host contains a SURT delimiter, has an unclosed bracket, or has text after its closing
    /// bracket that cannot be interpreted as a port.
    ...
    /// The text contains ASCII whitespace or a control character.
...
/// The canonicalization rules applied to a URL before it is transformed into a [`Surt`].
///
/// Regardless of its flags, every canonicalizer performs the following operations:
///
/// - repair input as the Python [surt] library does, by trimming whitespace,
///   removing tabs and newlines, defaulting to `http://`, and unwrapping `http://https://...`;
/// - lowercase the scheme, host, path, and query;
/// - drop user information, fragments, default ports, and empty queries;
/// - resolve path dot segments and strips trailing dots from hosts; and
/// - render numeric IPv4 hosts as dotted quads and non-ASCII host labels with Punycode.
///
/// [surt]: https://github.com/internetarchive/surt
#[expect(
...
/// How a SURT key writes an IPv6 host.
///
/// This is a rendering choice, not a canonicalization rule: the two forms name the same host, and
/// [`Surt::url_with_scheme`] restores the brackets either way.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
enum Ipv6Host {
    /// Keep the brackets the key's own syntax expects.
    Bracketed,
    /// Drop the brackets, as the Wayback Machine and the Python [surt] library do.
    ///
    /// [surt]: https://github.com/internetarchive/surt
    Bare,
}
...
impl<'a> Url<'a> {
    ...
    /// # Errors
    ///
    /// Fails when the text has no scheme or authority, when the host is missing or malformed, when
    /// the port is not a number, or when the text contains ASCII whitespace or control characters.
    ...
    /// Wayback Machine does; IPv6 literals are kept whole, without brackets). The user information
    /// and fragment are dropped, and a path is supplied as `/` when there is none
    /// but there is a query. No other canonicalization is applied; see [`Canonicalizer`] for that.
    /// Use [`Url::surt_ipv6_bracketed`] to keep an IPv6 host's brackets.
    #[must_use]
    pub fn surt(&self) -> Surt<'static> {
        self.surt_with_ipv6_host(Ipv6Host::Bare)
    }

    /// The SURT key, keeping an IPv6 host's brackets.
    ///
    /// This differs from [`Url::surt`] only for IPv6 hosts. Both forms name the same host, and
    /// [`Surt::url_with_scheme`] restores the brackets either way.
    #[must_use]
    pub fn surt_ipv6_bracketed(&self) -> Surt<'static> {
        self.surt_with_ipv6_host(Ipv6Host::Bracketed)
    }

    fn surt_with_ipv6_host(&self, ipv6: Ipv6Host) -> Surt<'static> {
        let mut key = String::with_capacity(self.text.len());
        let host = self.host();

        match ipv6 {
            // The literal is `[`, the address, then `]`, so trimming both ends is exact.
            Ipv6Host::Bare if host.starts_with('[') => {
                key.push_str(host.trim_start_matches('[').trim_end_matches(']'));
            }
            _ => push_reversed_host(&mut key, host, false),
        }
    ...
    /// parenthesized with a trailing comma, and the `/` after the host appears only if the URL had
    /// one. The user information and fragment are dropped and the scheme is lowercased; lowercasing
    /// the rest is left to a canonicalizer, such as [`Canonicalizer::HERITRIX`].
    ///
    ...
    ///
    /// Keeps the scheme, user information, path, query, and fragment. Domain labels are reversed
    /// with a trailing comma; IP addresses are kept whole. Ports are rendered as decimal numbers,
    /// so leading zeros and empty port delimiters are not preserved. The `:` before the scheme
    /// appears only when a port is present.
    #[must_use]
...
mod tests {
    ...
    fn parsing_preserves_components(#[strategy = prop::url_parts()] parts: prop::UrlParts) {
    ...
    #[proptest::property_test]
    fn surt_forms_agree(#[strategy = prop::url_parts()] parts: prop::UrlParts) {
        let text = parts.to_string();
        let url = Url::parse(&text).unwrap();
        let key = url.surt_ipv6_bracketed();
        let reversed: Vec<&str> = parts.host.rsplit('.').collect();
        let joined = reversed.join(",");
        let heritrix_host = format!("://({joined},");
        let ssurt_prefix = format!("{joined},//");

        prop_assert_eq!(&url.surt(), &key);
