//! Sort-friendly URI Reordering Transform (SURT) keys and the URL canonicalization that precedes
//! them.
//!
...
//! `com,example)/path?query`, which this crate represents as [`Surt`]. The [SSURT][ssurt] variant
//! is available from [`url::Url::ssurt`].
//!
//! Two URLs share a key only if they were canonicalized the same way, so a
//! [`url::Canonicalizer`] defines which rules apply. Its default,
//! [`url::Canonicalizer::WAYBACK`], reproduces the Wayback Machine's `urlkey` (the Python `surt`
//! library, as used by pywb, `cdxj-indexer`, and `py-wacz`), which [`Surt::from_url`] applies:
//!
//! ```
...
/// follows the address is part of the host rather than a port. Keys sort lexicographically; match
/// complete labels to avoid including unrelated domains that share a partial label.
///
...
impl<'a> Surt<'a> {
    ...
    ///
    /// Parses the text rendered by [`url`](Self::url) as a [`fluent_uri::Uri`].
    ///
    /// # Errors
    ///
    ...
    ///
    /// Parses the text rendered by [`url`](Self::url). The WHATWG parser may normalize or
    /// percent-encode that text, and its accepted input differs from [`to_uri`](Self::to_uri).
    ///
    /// # Errors
    ///
