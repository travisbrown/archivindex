...
impl<'a> Document<'a> {
    ...
    pub fn into_captures(self) -> Result<CaptureList<'a>, Error> {
        ...
        let values = records
            ...
            .enumerate()
            .map(|(record, values)| {
                crate::capture::from_fields(semantic_fields.iter().cloned().zip(values))
                    .map_err(|source| Error::Field { record, source })
            })
            .collect::<Result<_, _>>()?;
...
mod tests {
    ...
    fn decodes_wayback_aliases_and_unusual_values() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(matches!(list.values[0].key, Cow::Borrowed(_)));
        assert!(matches!(list.values[0].url, Cow::Borrowed(_)));
        assert!(matches!(list.values[0].mime, Some(Cow::Borrowed(_))));
        assert_eq!(
    ...
    #[test]
    fn consuming_owned_rows_reuses_retained_string_buffers() {
        let input = concat!(
            "[[\"urlkey\",\"timestamp\",\"url\",\"mime\",\"digest\",\"redirect\",",
            "\"robotflags\",\"filename\",\"recordDigest\",\"orig.filename\",\"custom\"],",
            "[\"com,example)/\",\"20201007212236\",\"https://example.com/\",",
            "\"text/html\",\"payload\",\"https://example.com/next\",\"A\",",
            "\"capture.warc.gz\",\"record\",\"original.warc.gz\",\"extra value\"]]"
        );
        let document = serde_json::from_str::<Document<'_>>(input)
            .unwrap()
            .into_owned();
        let pointers: Vec<_> = document.records()[0]
            .iter()
            .map(|value| value.as_ptr())
            .collect();
        let list = document.into_captures().unwrap();
        let capture = &list.values[0];
        let retained = [
            (0, &capture.key),
            (2, &capture.url),
            (3, capture.mime.as_ref().unwrap()),
            (4, capture.digest.as_ref().unwrap()),
            (5, capture.redirect.as_ref().unwrap()),
            (6, capture.robot_flags.as_ref().unwrap()),
            (7, capture.filename.as_ref().unwrap()),
            (8, capture.record_digest.as_ref().unwrap()),
            (
                9,
                capture
                    .original
        ...
        for (index, value) in retained {
            ...
            assert_eq!(value.as_ptr(), pointers[index]);
        }
        assert_eq!(
            capture.extra["custom"].as_str().unwrap().as_ptr(),
            pointers[10]
        );
    }
    ...
    fn document_round_trips(
        #[strategy = proptest::collection::vec(prop::capture_parts(), 0..=3)] rows: Vec<
            prop::CaptureParts,
        >,
        #[strategy = proptest::option::of(prop::json_text())] resume_key: Option<String>,
    ) {
        let document = Document::new(
            NAMES.iter().copied().map(Cow::Borrowed).collect(),
            rows.iter().map(prop::CaptureParts::values).collect(),
    ...
    #[proptest::property_test]
    fn captures_recover_every_value(#[strategy = prop::capture_parts()] parts: prop::CaptureParts) {
        let document = Document::new(
            NAMES.iter().copied().map(Cow::Borrowed).collect(),
            vec![parts.values()],
            None,
