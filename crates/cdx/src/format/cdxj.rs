...
/// A single CDXJ line.
///
/// [`Item`] uses lenient [`Fields`] by default. [`ConformingItem`] requires the seven mandatory
/// CDXJ 0.1.0 fields; `recordDigest` remains optional.
...
mod tests {
    ...
    fn text_round_trips(#[strategy = prop::item()] item: Item<'static>) {
    ...
    fn numbers_and_numeric_strings_agree(
        ...
        offset: u64,
        length: u64,
        #[strategy = prop::json_text()] filename: String,
    ) {
    ...
    fn conforming_fields_survive_the_lenient_form(
        #[strategy = prop::conforming_fields()] fields: ConformingFields<'static>,
    ) {
