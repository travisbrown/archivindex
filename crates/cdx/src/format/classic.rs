...
///
/// Classic headers may start with the delimiter (for example, ` CDX N b a`) or directly with `CDX`.
/// Parsing preserves whether the header begins with the delimiter, and formatting reproduces that
/// form.
...
impl<'a> Header<'a> {
    ...
    ///
    /// This is the only way to make a [`Record`], so every record matches the legend of the header
    /// that produced it.
    pub fn record<'b>(&self, values: Vec<Cow<'b, str>>) -> Result<Record<'b>, Error> {
    ...
    pub fn parse_record<'b>(&self, line: &'b str) -> Result<Record<'b>, Error> {
        self.record(line.split(self.delimiter).map(Cow::Borrowed).collect())
    }
    ...
    pub fn render(&self, record: &Record<'_>) -> String {
        join(self.delimiter, &record.values)
    }
...
/// The values of a classic CDX record, as many as the legend of the [`Header`] that made it.
///
/// [`Header::record`] and [`Header::parse_record`] are the only constructors, so that count always
/// holds.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Record<'a> {
...
mod tests {
    ...
    fn header_and_record_round_trip(
        ...
        .unwrap();
        let record = header
            .record(values.into_iter().map(Cow::Owned).collect())
            .unwrap();
        let header_text = header.to_string();
        let record_text = header.render(&record);
    ...
    fn the_standard_legend_recovers_every_value(
        #[strategy = prop::capture_parts()] parts: prop::CaptureParts,
    ) {
