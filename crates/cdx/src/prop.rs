...
use archivindex_test_support::prop::tokens_of;
...
impl CaptureParts {
    /// These values in the order of the standard 11-field legend (`N b a m s k r M S V g`), with
    ...
    /// These values as a classic CDX record under the standard 11-field legend.
    pub fn record(&self) -> Record<'static> {
        Header::standard_11()
            .record(self.values())
            .expect("the standard legend has one marker per value")
    }
}
