...
///
/// Known aliases from CDXJ, the CDX server API, and classic CDX legends map to dedicated variants.
/// Any other name or legend marker is retained verbatim.
...
impl<'a> Field<'a> {
    ...
    /// Interpret a marker from a classic CDX legend, which may be a name.
    ///
    /// Single characters are classic markers, anything longer is a name. Legends written by other
    /// tools mix the two.
    #[must_use]
    pub fn legend(marker: &'a str) -> Self {
    ...
    /// The canonical field name used by CDXJ and CDX server JSON, or the name an unmodeled field
    /// appeared with.
    #[must_use]
...
mod tests {
    ...
    fn names_are_canonical(#[strategy = prop::bare_text()] name: String) {
    ...
    #[proptest::property_test]
    fn unmodeled_names_are_borrowed_then_owned(#[strategy = prop::bare_text()] name: String) {
        let field = Field::named(&name);
