...
/// [`borrowable_str::seq`] for fields that `#[serde(borrow)]` cannot reach into. Serializing
/// writes the string itself, so a type holding one round-trips.
///
/// # Examples
///
/// ```
...
mod tests {
    ...
    /// A type using the wrapper directly must be able to derive both halves, which is what makes
    /// it usable as a field type rather than only as a `deserialize_with` target.
    #[test]
