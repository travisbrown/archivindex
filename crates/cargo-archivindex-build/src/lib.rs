...
"#;
    ...
    #[test]
    fn sync_repairs_shared_policy_and_preserves_unrelated_settings() {
        let fixture = Fixture::new();
        fixture.replace(
            "Cargo.toml",
            "rust_2018_idioms = { level = \"warn\", priority = -1 }",
        ...
        assert!(manifest.contains("rust_2018_idioms = { level = \"warn\", priority = -1 }"));
        assert!(manifest.contains("# Local lint\nunwrap_used = \"warn\""));
        assert!(
            fixture
                .read("rustfmt.toml")
                .contains("# Project style\nmax_width = 90")
        );
        let taplo = fixture.read(".taplo.toml");
        assert!(taplo.contains("# Project exclusions"));
        assert!(taplo.contains("\"vendor/**\""));
        assert!(taplo.contains("\"target/**\""));
    ...
    #[test]
    fn sync_adds_table_alignment_to_an_existing_prose_configuration() {
        let fixture = Fixture::new();
        let prose = RUMDL
            .split("\n[MD060]")
            .next()
            .expect("the fixture has prose settings")
            .replace("[\"MD013\", \"MD060\"]", "[\"MD013\"]");
        fixture.write(".rumdl.toml", &prose);

        let messages = fixture.messages(".rumdl.toml");
        assert!(mentions(&messages, "global.enable"));
        assert!(mentions(&messages, "MD060.enabled"));
        assert!(mentions(&messages, "MD060.style"));
        assert!(mentions(&messages, "MD060.max-width"));
    ...
    fn reports_incomplete_metadata_only_on_publishable_packages() {
        ...
        fixture.replace("member/Cargo.toml", "readme = \"README.md\"\n", "");
        fixture.replace(
            "member/Cargo.toml",
            "all-features = true",
            "all-features = false",
