...
use crate::policy::{
    ...
    DOCS_RS_ARGUMENTS, DOCS_RS_RUSTDOC_ARGS, INHERITED_PACKAGE_FIELDS, LINTS, PUBLISHED_FIELDS,
    RUMDL_BOOLEAN_SETTINGS, RUMDL_ENABLED_RULES, RUMDL_INTEGER_SETTINGS, RUMDL_STRING_SETTINGS,
    RUSTFMT_STRING_SETTINGS, TAPLO_INTEGER_SETTINGS, WORKSPACE_PACKAGE_FIELDS, WORKSPACE_RESOLVER,
};
...
/// Report the first pair of workspace dependencies that is out of order. `sync` sorts the table.
...
/// Check the metadata required for packages that allow publication.
...
fn lints(document: &DocumentMut, path: &Path, violations: &mut Vec<Violation>) {
    for (item_path, level, priority) in LINTS {
        if let Some(priority) = priority {
            check_string(
                document,
                &[item_path, &["level"]].concat(),
                level,
                path,
                violations,
            );
            check_integer(
                document,
                &[item_path, &["priority"]].concat(),
                priority,
                path,
                violations,
            );
        } else {
            check_string(document, item_path, level, path, violations);
        }
    }
}
...
fn rustfmt(project: &Project, violations: &mut Vec<Violation>) -> Result<(), Error> {
    ...
    for (item_path, expected) in RUSTFMT_STRING_SETTINGS {
...
fn taplo(project: &Project, violations: &mut Vec<Violation>) -> Result<(), Error> {
    ...
    for (item_path, expected) in TAPLO_INTEGER_SETTINGS {
        check_integer(&document, item_path, expected, &path, violations);
    }
    Ok(())
}

/// Check the shared `cargo deny` settings, leaving project-specific allowances unchanged.
...
fn rumdl(project: &Project, violations: &mut Vec<Violation>) -> Result<(), Error> {
    ...
    if !enabled {
        push(
            violations,
            &path,
            format!("`global.enable` must be {RUMDL_ENABLED_RULES:?}"),
        );
    }
    for (item_path, expected) in RUMDL_INTEGER_SETTINGS {
        check_integer(&document, item_path, expected, &path, violations);
    }
    for (item_path, expected) in RUMDL_STRING_SETTINGS {
        check_string(&document, item_path, expected, &path, violations);
    }
    for (item_path, expected) in RUMDL_BOOLEAN_SETTINGS {
        if get(&document, item_path).and_then(Item::as_bool) != Some(expected) {
            push(
                violations,
                &path,
                format!("`{}` must be {expected}", item_path.join(".")),
            );
        }
    }
    Ok(())
