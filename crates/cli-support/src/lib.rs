...
/// [`Ordering::Relaxed`] and stop between steps. A second signal exits immediately with status 130,
/// so a run whose current step never returns can still be stopped.
///
/// Installation is attempted only on the first call. If it fails, this function logs a warning and
/// the flag remains unset.
...
/// A count followed by `noun`, with `s` appended unless the count is one.
///
/// Use only with nouns whose plural is formed by adding `s`. The bound accepts every integer type
/// a count is likely to arrive in, so a `usize` length and a `u64` total need no conversion.
#[must_use]
pub fn plural<N: Copy + Display + PartialEq + From<u8>>(count: N, noun: &str) -> String {
...
/// `--quiet` logs only errors. Commands can use [`Verbosity::is_quiet`] to suppress their own
/// output.
#[derive(Debug, clap::Args)]
// Without this, clap would describe every command that flattens `Verbosity` with the doc comment
// above, since a flattened `Args` struct contributes its own description.
...
impl Verbosity {
    /// Whether quiet mode was requested. Callers decide which output to suppress.
    #[must_use]
...
mod tests {
    ...
    fn plural_agrees_with_its_count() {
        assert_eq!(super::plural(0, "record"), "0 records");
        assert_eq!(super::plural(1, "record"), "1 record");
        assert_eq!(super::plural(2, "byte"), "2 bytes");
    }

    /// Counts reach a command as whatever width its source produced, so all of them have to work
    /// without a cast at the call site.
    #[test]
    fn plural_accepts_any_width_of_count() {
        assert_eq!(super::plural(1usize, "file"), "1 file");
        assert_eq!(super::plural(2u64, "file"), "2 files");
        assert_eq!(super::plural(3u32, "file"), "3 files");
        assert_eq!(super::plural(4i64, "file"), "4 files");
        assert_eq!(
            super::plural(u64::MAX, "file"),
            "18446744073709551615 files"
        );
    }
