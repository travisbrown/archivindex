...
pub fn spinner(message: impl Into<Cow<'static, str>>, unit: Option<&str>) -> ProgressBar {
    ...
    configure(ProgressBar::new_spinner(), style, message, unit)
}
...
pub fn bar(length: u64, message: impl Into<Cow<'static, str>>, unit: Option<&str>) -> ProgressBar {
    ...
    configure(ProgressBar::new(length), style, message, unit)
}

/// Apply `style` and `message` to `progress`, holding `unit` in the prefix the templates read it
/// from.
fn configure(
